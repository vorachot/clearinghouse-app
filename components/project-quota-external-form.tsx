"use client";

import { useState, useEffect } from "react";
import { Button } from "@heroui/button";
import { Input, Textarea } from "@heroui/input";
import { Select, SelectItem } from "@heroui/select";
import { Slider } from "@heroui/slider";
import { Card, CardBody, CardHeader } from "@heroui/card";
import {
  Modal,
  ModalContent,
  ModalHeader,
  ModalBody,
  ModalFooter,
} from "@heroui/modal";
import {
  Table,
  TableHeader,
  TableBody,
  TableColumn,
  TableRow,
  TableCell,
} from "@heroui/table";
import { CreateProjectQuotaDTO, OrganizationQuota } from "@/types/quota";

type ProjectQuotaExternalFormProps = {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateProjectQuotaDTO) => void;
  orgQuotas: OrganizationQuota[];
  projectId: string;
};

type ResourceFormItem = {
  resourceId: string;
  resourceName: string;
  resourceTypeName: string;
  quantity: number;
  maxQuantity: number;
  price: number;
  duration: number;
};

export default function ProjectQuotaExternalForm({
  isOpen,
  onClose,
  onSubmit,
  orgQuotas,
  projectId,
}: ProjectQuotaExternalFormProps) {
  const [formData, setFormData] = useState({
    name: "",
    description: "",
    orgQuotaId: "",
  });

  const [resources, setResources] = useState<ResourceFormItem[]>([]);
  const [selectedOrgQuota, setSelectedOrgQuota] =
    useState<OrganizationQuota | null>(null);

  // Auto-populate resources when org quota is selected
  useEffect(() => {
    if (formData.orgQuotaId) {
      const quota = orgQuotas.find((q) => q.id === formData.orgQuotaId);
      setSelectedOrgQuota(quota || null);

      if (quota && quota.resources.length > 0) {
        setResources(
          quota.resources.map((resource) => ({
            resourceId: resource.resource_prop.resource.id,
            resourceName: resource.resource_prop?.resource?.name || "Unknown",
            resourceTypeName:
              resource.resource_prop?.resource?.resource_type?.name ||
              "Unknown",
            quantity: 0,
            maxQuantity: resource.quantity,
            price: resource.resource_prop?.price || 0.01,
            duration: (resource.resource_prop?.max_duration || 3600) / 3600, // Convert seconds to hours
          }))
        );
      } else {
        setResources([]);
      }
    } else {
      setSelectedOrgQuota(null);
      setResources([]);
    }
  }, [formData.orgQuotaId, orgQuotas]);

  const handleResourceChange = (
    resourceId: string,
    field: keyof ResourceFormItem,
    value: string | number
  ) => {
    setResources(
      resources.map((r) =>
        r.resourceId === resourceId ? { ...r, [field]: value } : r
      )
    );
  };

  const handleSubmit = () => {
    if (!selectedOrgQuota) return;

    const dto: CreateProjectQuotaDTO = {
      name: formData.name,
      description: formData.description || "-",
      project_id: projectId,
      org_quota_id: formData.orgQuotaId,
      org_id: selectedOrgQuota.to_organization_id,
      node_id: selectedOrgQuota.node_id,
      resources: resources
        .filter((r) => r.quantity > 0)
        .map((r) => ({
          resource_id: r.resourceId,
          quantity: r.quantity,
          price: r.price,
          duration: r.duration * 3600, // Convert hours to seconds
        })),
    };

    onSubmit(dto);
    handleReset();
  };

  const handleReset = () => {
    setFormData({
      name: "",
      description: "",
      orgQuotaId: "",
    });
    setResources([]);
    setSelectedOrgQuota(null);
  };

  const isFormValid =
    formData.name &&
    formData.orgQuotaId &&
    resources.length > 0 &&
    resources.some((r) => r.quantity > 0) &&
    resources.filter((r) => r.quantity > 0).every((r) => r.price >= 0);

  return (
    <Modal isOpen={isOpen} onClose={onClose} size="5xl" scrollBehavior="inside">
      <ModalContent>
        <ModalHeader className="dark:text-white">
          Create External Project Quota
        </ModalHeader>
        <ModalBody>
          <div className="space-y-4">
            {/* Basic Info */}
            <Card>
              <CardHeader>
                <h3 className="dark:text-white text-lg font-semibold">
                  Basic Information
                </h3>
              </CardHeader>
              <CardBody className="space-y-4">
                <Input
                  label="Name"
                  placeholder="Enter quota name"
                  value={formData.name}
                  onValueChange={(value) =>
                    setFormData({ ...formData, name: value })
                  }
                  isRequired
                />
                <Textarea
                  label="Description"
                  placeholder="Enter quota description"
                  value={formData.description}
                  onValueChange={(value) =>
                    setFormData({ ...formData, description: value })
                  }
                />
              </CardBody>
            </Card>

            {/* Organization Quota Selection */}
            <Card>
              <CardHeader>
                <h3 className="text-lg font-semibold">
                  Organization Quota Selection
                </h3>
              </CardHeader>
              <CardBody className="space-y-4">
                <Select
                  label="Organization Quota"
                  placeholder="Select organization quota"
                  selectedKeys={
                    formData.orgQuotaId ? [formData.orgQuotaId] : []
                  }
                  onSelectionChange={(keys) => {
                    const key = Array.from(keys)[0] as string;
                    setFormData({ ...formData, orgQuotaId: key });
                  }}
                  description="Select an organization quota to allocate resources from"
                  isRequired
                >
                  {orgQuotas.map((quota) => (
                    <SelectItem
                      key={quota.id}
                      textValue={quota.name}
                      className="dark:text-white"
                    >
                      <div>
                        <p className="font-semibold">{quota.name}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {quota.from_organization?.name || "Unknown"} →{" "}
                          {quota.to_organization?.name || "Unknown"}
                        </p>
                      </div>
                    </SelectItem>
                  ))}
                </Select>

                {selectedOrgQuota && (
                  <Input
                    label="Node"
                    value={selectedOrgQuota.node_id}
                    isReadOnly
                    description="Node is auto-filled from organization quota"
                  />
                )}
              </CardBody>
            </Card>

            {/* Resources */}
            <Card>
              <CardHeader>
                <h3 className="text-lg font-semibold">
                  Configure Resource Allocation
                </h3>
              </CardHeader>
              <CardBody>
                {resources.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">
                    {formData.orgQuotaId
                      ? "No resources available in this organization quota"
                      : "Select an organization quota to view available resources"}
                  </p>
                ) : (
                  <Table aria-label="Resources table">
                    <TableHeader>
                      <TableColumn>RESOURCE TYPE</TableColumn>
                      <TableColumn>NAME</TableColumn>
                      <TableColumn>AVAILABLE</TableColumn>
                      <TableColumn>QUANTITY</TableColumn>
                      <TableColumn>PRICE (credits/hr)</TableColumn>
                      <TableColumn>DURATION (hrs)</TableColumn>
                      <TableColumn>TOTAL</TableColumn>
                    </TableHeader>
                    <TableBody>
                      {resources.map((resource) => (
                        <TableRow key={resource.resourceId}>
                          <TableCell>
                            <span className="font-medium">
                              {resource.resourceTypeName}
                            </span>
                          </TableCell>
                          <TableCell>
                            <span className="font-medium">
                              {resource.resourceName}
                            </span>
                          </TableCell>
                          <TableCell>
                            <span className="text-sm text-gray-600 dark:text-gray-400">
                              {resource.maxQuantity}
                            </span>
                          </TableCell>
                          <TableCell>
                            <div className="flex flex-col gap-2 min-w-[200px]">
                              <Slider
                                size="sm"
                                step={1}
                                minValue={0}
                                maxValue={resource.maxQuantity}
                                value={resource.quantity}
                                onChange={(value) =>
                                  handleResourceChange(
                                    resource.resourceId,
                                    "quantity",
                                    value as number
                                  )
                                }
                                className="max-w-md"
                              />
                              <span className="text-xs text-default-500">
                                {resource.quantity} / {resource.maxQuantity}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell>
                            <Input
                              type="number"
                              min="0"
                              step="0.01"
                              value={resource.price.toString()}
                              onValueChange={(value) =>
                                handleResourceChange(
                                  resource.resourceId,
                                  "price",
                                  parseFloat(value) || 0
                                )
                              }
                              size="sm"
                              className="w-24"
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              type="number"
                              min="0.5"
                              step="0.5"
                              value={resource.duration.toString()}
                              onValueChange={(value) =>
                                handleResourceChange(
                                  resource.resourceId,
                                  "duration",
                                  parseFloat(value) || 1
                                )
                              }
                              size="sm"
                              className="w-24"
                            />
                          </TableCell>
                          <TableCell>
                            <span className="font-semibold">
                              {(
                                resource.quantity *
                                resource.price *
                                resource.duration
                              ).toFixed(2)}
                            </span>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                )}
              </CardBody>
            </Card>

            {/* Total Summary */}
            {resources.length > 0 && resources.some((r) => r.quantity > 0) && (
              <Card>
                <CardBody>
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-semibold">Total Price:</span>
                    <span className="text-2xl font-bold text-primary">
                      {resources
                        .filter((r) => r.quantity > 0)
                        .reduce(
                          (sum, r) => sum + r.quantity * r.price * r.duration,
                          0
                        )
                        .toFixed(2)}{" "}
                      credits
                    </span>
                  </div>
                </CardBody>
              </Card>
            )}
          </div>
        </ModalBody>
        <ModalFooter>
          <Button variant="light" onPress={onClose}>
            Cancel
          </Button>
          <Button
            color="primary"
            onPress={handleSubmit}
            isDisabled={!isFormValid}
          >
            Create Quota
          </Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
  );
}
